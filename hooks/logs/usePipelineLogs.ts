...
import {
  getAllUserBuildHistories,
  transformBuildToLogItem,
  searchBuildHistories,
} from "@/lib/api/pipeline-logs";
    ...
    async (
      ...
      try {
        ...
        if (searchQuery.trim()) {
          ...
          const transformedLogs = result.builds.map((build) =>
            transformBuildToLogItem(build)
          );
          ...
          result = await getAllUserBuildHistories(currentUserId, {
            ...
            status: filters.status,
            projectId:
              filters.branch !== "all-branches" ? filters.branch : undefined,
          });
