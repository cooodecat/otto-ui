...
const NotificationEmailNode = memo(({ data, id }: NodeProps) => {
  ...
  const [subject, setSubject] = useState(
    nodeData.subjectTemplate || "Build Status: {status}"
  );
  const [body, setBody] = useState(
    nodeData.bodyTemplate ||
      "Project {project} on {branch} finished with {status}."
  );
    ...
    >
      <div className="space-y-3 text-sm">
        ...
        <div>
          ...
          <textarea
            className="w-full px-2 py-1 border border-gray-300 rounded h-24"
            value={body}
