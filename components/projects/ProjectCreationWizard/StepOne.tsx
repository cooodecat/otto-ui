            ...
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                ...
                <span className="inline-flex items-center gap-1">
                  {repository.visibility === 'Public' ? <Globe className="w-3 h-3" /> : <Lock className="w-3 h-3" />}
                  {repository.visibility}
                </span>
              </span>
            </div>
            <p className="text-gray-600 text-sm">{repository.description || '설명이 없습니다'}</p>
          </div>
        </div>
        ...
        {topLanguages.length > 0 && (
          <div className="border-t border-gray-200 pt-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-sm font-medium text-gray-700">주요 언어</span>
            </div>
            <div className="space-y-2">
              {topLanguages.map((lang, index) => (
                <div key={lang.name} className="flex items-center gap-3">
                  <span className="text-sm text-gray-600 w-20">{lang.name}</span>
                  ...
                  <span className="text-sm text-gray-500 w-12 text-right">{lang.percentage}%</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        ...
        <div className="relative">
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span className="text-gray-900">{selectedBranch || '브랜치를 선택하세요'}</span>
            </div>
                  ...
                  filteredBranches.map((branch) => (
                    <button
                      ...
                      className={`w-full px-4 py-2.5 text-left hover:bg-gray-50 transition-colors ${
                        branch.name === selectedBranch ? 'bg-purple-50 text-purple-700' : 'text-gray-700'
                      }`}
                    >
        ...
        {selectedBranchData && selectedBranchData.commit && (
          <div className="mt-4 p-4 bg-purple-50 rounded-lg border border-purple-200">
            <div className="text-sm font-medium text-purple-900 mb-1">최신 커밋</div>
            <div className="text-sm text-purple-700">{selectedBranchData.commit.message}</div>
