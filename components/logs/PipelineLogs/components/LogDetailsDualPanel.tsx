...
import { 
  ...
  Play, Square, ArrowDownToLine,
  ChevronLeft, ChevronRight, 
  Clock, GitBranch, User, Hash, Filter, Search
} from 'lucide-react';
  ...
  return (
    <Dialog.Root open={isOpen} onOpenChange={onClose}>
      <Dialog.Portal>
        ...
        <Dialog.Content 
          className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-full max-w-[90vw] h-[85vh] bg-white rounded-xl shadow-2xl z-50 flex flex-col overflow-hidden"
        >
          {/* Header */}
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-gray-50 to-white">
            <div className="flex items-center gap-4">
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  <span className="text-sm text-gray-500">Full ID: {buildId}</span>
                  <button
                    onClick={copyBuildId}
            ...
            <div className="flex items-center gap-2">
              ...
              >
                <ArrowDownToLine className={cn('w-4 h-4', autoScroll ? 'text-blue-600' : 'text-gray-600')} />
              </button>
                    ...
                    <div className="bg-white rounded-lg p-3 border border-gray-200">
                      <div className="flex items-center justify-between">
                        ...
                        <span className={cn(
                          ...
                          buildStatus === 'SUCCEEDED' ? 'bg-green-100 text-green-700' :
                      ...
                      <>
                        <div className="bg-white rounded-lg p-3 border border-gray-200 space-y-2">
                          <div className="flex items-center gap-2 text-sm">
                            ...
                            <span className="font-mono text-gray-900">{buildMetadata.branch}</span>
                          </div>
                          <div className="flex items-center gap-2 text-sm">
                            <Hash className="w-4 h-4 text-gray-400" />
                            <span className="text-gray-600">Commit:</span>
                            <span className="font-mono text-gray-900">{buildMetadata.commit.slice(0, 7)}</span>
                          </div>
                          <div className="flex items-center gap-2 text-sm">
                            <User className="w-4 h-4 text-gray-400" />
                            <span className="text-gray-600">Author:</span>
                            <span className="text-gray-900">{buildMetadata.author}</span>
                          </div>
                          <div className="flex items-center gap-2 text-sm">
                            <Clock className="w-4 h-4 text-gray-400" />
                            <span className="text-gray-600">Duration:</span>
                            <span className="text-gray-900">{buildMetadata.duration || 'In progress...'}</span>
                          </div>
                        </div>
                      </>
                    )}
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      ...
                      className={cn(
                        ...
                        !selectedLevel ? 'bg-blue-50 border-blue-300 text-blue-700' : 'bg-white border-gray-200 text-gray-600 hover:bg-gray-50'
                      )}
                    >
                    ...
                    <button
                      ...
                      className={cn(
                        'p-2 rounded-lg border text-sm font-medium transition-all cursor-pointer',
                        selectedLevel === 'ERROR' ? 'bg-red-50 border-red-300 text-red-700' : 'bg-white border-gray-200 text-gray-600 hover:bg-gray-50'
                      )}
                    >
                    ...
                    <button
                      ...
                      className={cn(
                        'p-2 rounded-lg border text-sm font-medium transition-all cursor-pointer',
                        selectedLevel === 'WARN' ? 'bg-yellow-50 border-yellow-300 text-yellow-700' : 'bg-white border-gray-200 text-gray-600 hover:bg-gray-50'
                      )}
                    >
                    ...
                    <button
                      ...
                      className={cn(
                        'p-2 rounded-lg border text-sm font-medium transition-all cursor-pointer',
                        selectedLevel === 'INFO' ? 'bg-blue-50 border-blue-300 text-blue-700' : 'bg-white border-gray-200 text-gray-600 hover:bg-gray-50'
                      )}
                    >
                      Info: {logStats.info}
                    </button>
              ...
              >
                {filteredLogs.length === 0 ? (
                  ...
                  filteredLogs.map((line, idx) => {
                    ...
                    return (
                      <div key={`${line.ts}-${idx}`} className="mb-1 whitespace-pre-wrap break-words hover:bg-gray-900/50 px-2 py-0.5 rounded">
                        <span className="text-gray-500">[{new Date(line.ts).toLocaleTimeString()}]</span>
                        <span className={`ml-2 ${levelColor}`}>{line.level}</span>
                        {line.phase && (
                          <span className={`ml-2 inline-flex items-center px-1.5 py-0.5 rounded text-xs ${phaseColor}`}>
                            {line.phase}
                          </span>
                        )}
                        {line.source && (
                          <span className="ml-2 text-cyan-400">[{line.source}]</span>
                        )}
                        <span className="ml-2 text-gray-100">{line.message}</span>
                      </div>
                    );
                  })
                )}
