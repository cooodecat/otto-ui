...
const buttonStyles = {
  iconButton:
    "p-2 rounded-lg border border-gray-300 bg-white text-gray-700 hover:bg-gray-50 cursor-pointer transition-colors",
  toggleButton:
    "px-4 py-2 rounded-lg border font-medium transition-colors cursor-pointer",
};
 ...
 */
const PipelineLogsPage: React.FC<
      ...
      setDisplayedLogs((prev) => {
        ...
        if (uniqueNewLogs.length > 0) {
          ...
          if (firstLog.status === "failed" && preferences.showFailureToasts) {
            showError(
              `Pipeline Failed: ${firstLog.pipelineName}`,
              `Build #${firstLog.id} failed`
            );
          } else if (
            ...
            showSuccess(
              `Pipeline Succeeded: ${firstLog.pipelineName}`,
              `Build #${firstLog.id} completed`
            );
          } else if (preferences.showInfoToasts) {
            showInfo(
              `New logs received`,
              `${uniqueNewLogs.length} new log entries`
            );
          }
    ...
    async (enabled: boolean) => {
      ...
      if (enabled && useRealApi) {
        try {
          ...
          connectSSE();
          console.log(
            "🔴 Live mode enabled - collection and streaming started"
          );
        } catch (error) {
        ...
        try {
          ...
          console.log(
            "⏹️ Live mode disabled - collection and streaming stopped"
          );
          // Live 끌 때 현재 최상단을 lastSeen으로 저장
          try {
            if (typeof window !== "undefined" && displayedLogs[0]) {
              localStorage.setItem(
                makeStorageKey("lastSeenId"),
                displayedLogs[0].id
              );
            }
          } catch {}
        ...
        console.log(
          enabled
            ? "🔴 Live mode enabled (mock)"
            : "⏹️ Live mode disabled (mock)"
        );
      }
    },
    [
      ...
      {useRealApi && (
        <ConnectionStatus
          ...
          error={
            ...
            (sseHasError ? "SSE connection issue" : undefined)
          }
