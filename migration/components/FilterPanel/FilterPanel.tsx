...
import { FilterPanelProps, FilterOption, DropdownPosition } from '../../types';
import { useFilters } from '../../hooks/useFilters';

interface FilterPanelInternalProps extends FilterPanelProps {
  // Router 관련 props (새 프로젝트에서 주입)
  pathname?: string;
 ...
 * 
 * GlobalSidebar에서 추출한 필터 패널 컴포넌트
 * Timeline, Status, Trigger, Branch, Author 필터링 기능 제공
 */
const FilterPanel: React.FC<FilterPanelInternalProps> = ({
  onFiltersChange,
  initialFilters = {},
  className = '',
  pathname = '',
  ...
  const { filters, setFilter, resetFilters } = useFilters({
    initialFilters,
    syncWithURL: !!updateURL && !!parseURLParams,
    updateURL: updateURL ? (filters, path) => {
  ...
  // 스크롤 시 드롭다운 닫기
  useEffect(() => {
    const isAnyDropdownOpen = isTimelineOpen || isStatusOpen || isTriggerOpen || isBranchOpen || isAuthorOpen;
  ...
  const calculateDropdownHeight = (optionsCount: number): number => {
    ...
    const visibleScrollableOptions = Math.min(scrollableOptions, maxVisibleOptions);
    
    return headerHeight + (visibleScrollableOptions * optionHeight) + containerPadding;
  };

  // 드롭다운 위치 계산
  const calculateDropdownPosition = (element: HTMLElement, optionsCount: number): 'top' | 'bottom' => {
      ...
      <div className='relative overflow-visible' ref={ref}>
        ...
        {isOpen && createPortal(
          ...
          >
            ...
            {/* 스크롤 가능한 나머지 옵션들 */}
            {options.length > 1 && (
              <div className='overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]' style={{ maxHeight: '168px' }}>
                {options.slice(1).map((option, index) => (
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-3 py-2.5 text-sm hover:bg-gray-50 cursor-pointer transition-colors ${
                      index === options.slice(1).length - 1 ? 'rounded-b-lg' : ''
                    } ${
  ...
  return (
    <div className={`bg-white rounded-xl shadow-lg border border-gray-200 p-4 flex-1 flex flex-col min-h-0 overflow-visible filter-panel-container ${className}`}>
      <div className='flex-1 space-y-6 pr-1 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] filter-panel-content' style={{ overflowY: 'auto', overflowX: 'visible' }}>
        {/* Timeline Filter */}
          ...
          {renderDropdownOptions(
            ...
            () => {
              ...
              if (!currentState && timelineDropdownRef.current) {
                const position = calculateDropdownPosition(timelineDropdownRef.current, timelineOptions.length);
                setDropdownPositions(prev => ({ ...prev, timeline: position }));
              }
          ...
          {renderDropdownOptions(
            ...
            () => {
              ...
              if (!currentState && statusDropdownRef.current) {
                const position = calculateDropdownPosition(statusDropdownRef.current, statusOptions.length);
          ...
          {renderDropdownOptions(
            ...
            () => {
              ...
              if (!currentState && triggerDropdownRef.current) {
                const position = calculateDropdownPosition(triggerDropdownRef.current, triggerOptions.length);
                setDropdownPositions(prev => ({ ...prev, trigger: position }));
              }
          ...
          {renderDropdownOptions(
            ...
            () => {
              ...
              if (!currentState && branchDropdownRef.current) {
                const position = calculateDropdownPosition(branchDropdownRef.current, branchOptions.length);
          ...
          {renderDropdownOptions(
            ...
            () => {
              ...
              if (!currentState && authorDropdownRef.current) {
                const position = calculateDropdownPosition(authorDropdownRef.current, authorOptions.length);
