...
export async function getProjectBuildHistories(
  ...
  // 빌드 데이터에 프로젝트 정보 추가
  const buildsWithProject = (data || []).map(
    (build) =>
      ({
        ...(build as any),
        projects: project,
...
export async function getAllUserBuildHistories(
  ...
  const result = await retryApiCall(async () => {
    return await supabase
      .from("projects")
      .select(
        "project_id, name, github_repo_name, selected_branch, github_owner"
      )
      .eq("user_id", userId);
  });
  const { data: projects, error: projectsError } = result as {
    data: ProjectSelect[] | null;
    error: any;
  };
  ...
  const buildsWithProject = (data || []).map((build: any) => {
    const project = projects?.find(
      (p: any) => p.project_id === build.project_id
    );
    return {
      ...(build as any),
      projects: project
...
export async function searchBuildHistories(
  ...
  const { data: projects, error: projectsError } = searchResult as {
